...
const Sidebar = () => {
  ...
  const menuItems = [
    { path: '/dashboard', name: 'Home', icon: <HomeIcon className="h-5 w-5" /> },
    { path: '/products', name: 'Products', icon: <ShoppingBagIcon className="h-5 w-5" /> },
    { path: '/users', name: 'Users', icon: <UsersIcon className="h-5 w-5" /> },
    { path: '/sales', name: 'Sales', icon: <BarChartIcon className="h-5 w-5" /> }
    // { path: '/analytics', name: 'Analytics', icon: <BarChartIcon className="h-5 w-5" /> },
        ...
        >
          <div className="h-full flex flex-col overflow-y-auto scrollbar-thin">
            ...
            <div className={`mt-auto pt-4 border-t border-gray-200 ${!isOpen && 'lg:text-center'}`}>
              <ul className="space-y-1 px-2">
                <li title={!isOpen ? "Help & Support" : undefined}>
                  <Link
                    to="/help"
                    className={`flex items-center px-2 py-2 text-gray-700 rounded-lg hover:bg-gray-100 transition-all duration-200 ${
                      !isOpen && 'lg:justify-center'
                    }`}
                  >
